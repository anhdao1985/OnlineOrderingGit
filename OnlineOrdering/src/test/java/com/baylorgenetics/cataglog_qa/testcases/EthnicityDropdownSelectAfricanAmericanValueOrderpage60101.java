...
/*
 * String[] exp = {"African American", "Ashkenazi Jewish",
 * "East Asian (China, Japan, Korea)", "Finnish", "French Canadian",
 * "Hispanic American", "Mennonite",
 * "Middle Eastern (Saudi Arabia, Qatar, Iraq, Turkey", "Native American",
 * "Northern Eauropean Caucasian (Scandinavian, UK, Germany)",
 * "Pacific  Islander (Philippines, Micronesia, Malaysia, Indoneasia",
 * "South Asian (India, Pakistan)",
 * "Southeast Asian (Vietnam, Cambodia, Thailand)"
 * ,"Southern Eauropean Caucasian (Spain, Italy, Greece)", "Other"}; WebElement
 * dropdown = driver.findElement(By.xpath("//div[@id='select-month']")); Select
 * ethicity = new Select(dropdown);
 * 
 * List<WebElement> options = ethicity.getOptions(); loop: for(WebElement
 * we:options) { for (int i=0; i<exp.length; i++){ if
 * (we.getText().equals(exp[i])){ System.out.println("Matched"); break loop; } }
 * 
 * }
 * 
 * 
 ...
 * "South Asian (India, Pakistan)",
 * "Southeast Asian (Vietnam, Cambodia, Thailand)"
 * ,"Southern Eauropean Caucasian (Spain, Italy, Greece)", "Other"};
 * 
 * WebElement dropdown =
 * driver.findElement(By.xpath("//*[@id='select-ethnicityName']")); Select
 * ethicity = new Select(dropdown);
 * 
 * List<WebElement> options = ethicity.getOptions(); loop: for(WebElement
 * we:options) { for (int i=0; i<exp.length; i++){ if
...
/*
 * int count = 0; String[] exp = {"African American", "Ashkenazi Jewish",
 * "East Asian (China, Japan, Korea)", "Finnish", "French Canadian",
 * "Hispanic American", "Mennonite",
 * "Middle Eastern (Saudi Arabia, Qatar, Iraq, Turkey", "Native American",
 * "Northern Eauropean Caucasian (Scandinavian, UK, Germany)",
 ...
 * "Southeast Asian (Vietnam, Cambodia, Thailand)"
 * ,"Southern Eauropean Caucasian (Spain, Italy, Greece)", "Other"}; WebElement
 * divClick = driver.findElement("//div[@id='select-ethnicityName']");
 * divClick.click(); WebElement dropdown =
 * driver.findElement(By.xpath("//div[@id='select-ethnicityName']")); Select
 * select = new Select(dropdown); java.util.List<WebElement> options =
 * select.getOptions(); for(WebElement item:options) { for (int i=0;
 * i<exp.length; i++){ if (item.getText().equals(exp[i])){
