...
public class FemalePatientInfomation {
	...
	//	 Thread.sleep(1000);
		 ...
		 Thread.sleep(1000);
			...
			// below is the portion to randomly select the Year in the Year drop down	 
				...
				/*
				 Select the radio button randomly
				 List<WebElement> options = driver.findElements(By.xpath("//*[starts-with(@id, 'patinet-gender')]")) ;
				 Random randomGender = new Random();
				 int index = randomGender.nextInt(options.size());
		...
		// below is the portion to randomly select the ethnicity in the ethnicity drop down	 
				 //Object of the e
